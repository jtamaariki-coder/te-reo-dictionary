...
export default function Footer() {
  return (
    <footer className="mt-auto border-t" style={{ borderColor: 'var(--border)', background: 'var(--muted)' }}>
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {EXPLORE_LINKS.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm hover:underline" style={{ color: 'var(--accent)' }}>
                    {link.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-8 pt-6 border-t flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs"
          style={{ borderColor: 'var(--border)', color: 'var(--text-secondary)' }}>
          <p>Built with aroha for te reo Māori and all of Aotearoa.</p>
          <p>Data sourced from Te Aka and community contributions. All entries attributed.</p>
        </div>
      </div>
    </footer>
  )
